    ...
    this.resolve(this.relPath).then(
      (url) => {
        if (this.kind === "image") {
          ...
          img.addEventListener("click", () => openLightbox(url, name, "image"));
 ...
 */
export function attachmentPreviews(load: AttachmentLoader): Extension {
  ...
  function build(state: EditorState): DecorationSet {
    ...
    while ((m = LINK_RE.exec(text))) {
      ...
      widgets.push(
        Decoration.widget({
          widget: new MediaWidget(relPath, kind, resolve),
          block: true,
          side: 1,
        }).range(line.to),
      );
