...
export function App() {
  ...
  // On macOS, Tauri receives Finder drops as native filesystem paths before
  // WebKit can expose browser File objects. Forward each completed drop to the
  // focused note pane; that pane asks the Rust bridge to import the files.
  useEffect(() => {
    ...
    void getCurrentWebview()
      .onDragDropEvent(({ payload }) => {
        ...
        window.dispatchEvent(
          new CustomEvent("ship-memory:native-file-drop", {
      ...
      .catch((error) => console.warn("ShipMemory native drop listener failed", error));
    return () => {
      cancelled = true;
      unlisten?.();
    };
  ...
  const importDroppedAttachments = (paths: string[]) =>
    invoke<SavedAttachment[]>("import_dropped_attachments", {
      hubRoot: engine.root,
      paths,
    });
  ...
  return (
    <>
      <Notepad
        ...
        onImportDroppedAttachments={importDroppedAttachments}
